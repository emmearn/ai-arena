...
| Impatti | Richiede API key OpenAI server-side tramite variabile d'ambiente/secret manager quando l'adapter OpenAI viene attivato; nessuna password ChatGPT o credenziale utente e' richiesta. I timeout applicativi partono da `arena.ai.request-timeout=30s`; retry Spring AI iniziale limitato a 2 tentativi. Nessuna API key viene salvata in repo o inviata al frontend. |
